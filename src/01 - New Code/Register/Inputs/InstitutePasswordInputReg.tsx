...
export const InstitutePasswordInputReg = () => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (validatePassword(inputValue)) {
      ...
      dispatch(postAuthRegister({ password: pass }));
      setCheckPassword(true)
    } else {
      dispatch(postAuthRegister({ password: "" }));
  ...
  return (
    <div className="sm:w-[100%] w-[250px] flex flex-col gap-1  relative">
      ...
      placeholder="Enter Password..."
          ...
          className=" mt-0 p-2 w-[100%] border-[1px] focus:border-[2px] border-gray-300 rounded-md shadow-sm focus:outline-none border-solid focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50"
        />
      ...
      {!checkPassword && ( // Display error message when phone number is invalid
        <p className="text-red-800 ">
          *Your password must be 6 to 15 characters long and include at least one alphabet,
