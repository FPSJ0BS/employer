...
import { editEmployerEditJob, employerEditJob } from "../../../Redux/EmployerEditJob";
import { SelectionProcessInterface } from "../../../Redux/EmployerEditJob";

export const EditJobSelectionProcess = () => {
  const { selectionProcessData, employerEditJob } = useSelector((state: any) => state.employerEditJob);
  ...
  return (
    <div className=" relative  sm:w-[100%] w-[200px] ">
      <label
        htmlFor="EditJobSelectionProcess"
      ...
      <div className="relative">
        ...
        required
          ref={inputRef}
          type="text"
          id="EditJobSelectionProcess"
          name="EditJobSelectionProcess"
          value={employerEditJob?.selection_process}
      ...
      {showDropdown && (
        <ul className=" postjobHandleScrollbar max-h-[300px] overflow-y-auto absolute z-10 mt-1 w-full bg-white border border-gray-300 rounded-md shadow-lg">
          {inputValue
            ...
            : selectionProcessData.map((option:string, index:number) => (
                <li
                  key={index}
                  className="cursor-pointer hover:bg-gray-100 py-1 px-3"
                  onClick={() => handleOptionSelect(option)}
