...
export const EditJobState = ({ isClear }: { isClear?: boolean }) => {
  ...
  useEffect(() => {
    if (employerEditJob?.state) {
      setInputValue(employerEditJob.state);
      const filteredOptions = PostJobPreFillDataState.filter(
        (option: StateInterface) =>
          option.name
            .toLowerCase()
            .includes(employerEditJob?.state?.toLowerCase())
      );
      setDisplayedOptions(filteredOptions.slice(0, lazyLoadCount));
    }
  }, [employerEditJob?.state, PostJobPreFillDataState]);

  useEffect(() => {
    const fecthData = async () => {
      const res = await getStateListAxios();
      if (res?.data?.status) {
        dispatch(setStatesData(res?.data?.data));
      }
    };
    fecthData();
  }, []);
  ...
  useEffect(() => {
    employerEditJob.state && setIsSelect(true);
  }, []);

  return (
    <div className="relative sm:w-[100%] w-[200px]">
      <label
        htmlFor="EditJobState"
