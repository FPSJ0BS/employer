...
export const ShowDataLength = ({jobID}) => {
  const navigate = useNavigate()
  const dispatch = useDispatch();

  const { employerCandidateData } = useSelector((state: any) => state.employerCandidate);


  const [showDropdown, setShowDropdown] = useState(false);
  const inputRef = useRef<HTMLInputElement>(null);

  const allJobTypes = [10, 20, 50, 100, 500];
    ...
    <div className="relative  sm:w-[15%] w-[250px]">
      ...
      <div className="relative">
        <input
          ...
          value={employerCandidateData?.sortDataLength}
